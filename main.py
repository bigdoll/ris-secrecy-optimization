...
# Change your suffix to just “.ckpt”
CHECKPOINT_SUFFIX = '.ckpt'

# ----------------------------------------------------------------------------
# Checkpoint utilities
# ----------------------------------------------------------------------------
def load_or_init_checkpoint(filepath):
    """
    Load existing checkpoint dict from .npz or return empty dict if none.
    """
    if os.path.exists(filepath):
        data = np.load(filepath, allow_pickle=True)
        return data['checkpoint_data'].item()
    return {}
...
def save_checkpoint(data, filepath):
    ...
    tmp = f"{base}.tmp{ext}"      # e.g. “foo.ckpt.tmp.npz”

    np.savez_compressed(tmp, checkpoint_data=data)
    os.replace(tmp, filepath)     # atomic rename
    
# def save_checkpoint(data, filepath):
#     """
#     Atomically save checkpoint dict to .npz.
#     """
#     tmp = filepath + '.tmp'
#     np.savez_compressed(tmp, checkpoint_data=data)
...
# def save_checkpoint(data, filepath):
#     """
#     Atomically save checkpoint dict to a .npz file.
#     - Creates parent directories if needed.
#     - Writes to a .tmp.npz file first, then renames.
#     """
#     # 1) Ensure the target directory exists
#     out_dir = os.path.dirname(os.path.abspath(filepath))
#     os.makedirs(out_dir, exist_ok=True)

#     # 2) Build a temp-filename with the same extension
#     base, ext = os.path.splitext(filepath)
#     # ext should be ".npz"; tmp becomes "basename.tmp.npz"
#     tmp = f"{base}.tmp{ext}"

#     # 3) Save to the temp file (np.savez_compressed respects the .npz ext)
#     np.savez_compressed(tmp, checkpoint_data=data)

#     # 4) Atomically replace the old checkpoint
#     os.replace(tmp, filepath)


def make_task_key(sample_index, params_combo):
    """
    Create a unique key for each simulation task.
    """
    combo_json = json.dumps(params_combo, sort_keys=True)
    return f"{sample_index}_|_{combo_json}"
...
def simulation(sample_index, config, channel_samples_range, ris_samples_range, params_combo, p_init_pcsi, gamma_init_pcsi, p_init_scsi, gamma_init_scsi):
    ...
    optimizer_psci = Optimizer(H, G_B, gE_hat, gE_error, config.sigma_sq, config.sigma_RIS_sq, sigma_e_sq, config.mu, Pc, PRmax, config.a, config.Ptmax, config.BW, scsi_bool=0) # gE_hat
    optimizer_scsi = Optimizer(H, G_B, gE_hat, gE_error, config.sigma_sq, config.sigma_RIS_sq, sigma_e_sq, config.mu, Pc, PRmax, config.a, config.Ptmax, config.BW, scsi_bool=1) # gE_hat
    ...
    key = make_task_key(sample_index, params_combo)
    return key, output_data, p_sol_pcsi, gamma_sol_pcsi, p_sol_scsi, gamma_sol_scsi
    # return output_data, p_sol_pcsi, gamma_sol_pcsi, p_sol_scsi, gamma_sol_scsi

# ----------------------------------------------------------------------------
# Coordinator for parallel tasks with checkpointing
# ----------------------------------------------------------------------------
def run_simulation(config):
    print("################################# STARTING SIMULATION: LET'S GO! #################################")
    # checkpoint_file = config.OUTPUT_FILE + CHECKPOINT_SUFFIX
    # build the checkpoint filename:
    base, ext = os.path.splitext(config.OUTPUT_FILE)
    # ext is “.npz”
    checkpoint_file = f"{base}{CHECKPOINT_SUFFIX}{ext}"
    checkpoint_data = load_or_init_checkpoint(checkpoint_file)

    num_processes = cpu_count()
    print(f"Using {num_processes} cores for parallel simulations.")

    # Generate channel & RIS samples
    ...
    channel_samples_range = Utils.monte_carlo_simulation(
        config.NUM_SAMPLES, config.FILENAME_CHANNEL, N_range,
        config.r_cell, config.h_BS, config.h_RIS,
        config.hmin_UE, config.hmax_UE, config.d0,
        config.K, config.NR, config.f0,
        config.lambda_0, config.d, config.c,
        config.n, config.R_K, NEEV_range,
        config.vic_percent_eve,
        channel_model="rician", shadowing=False
    )
    ris_samples_range = Utils.generate_ris_coefficients(
        config.NUM_SAMPLES, config.FILENAME_RIS,
        N_range, NEEV_range,
        channel_samples_range,
        (config.PTMIN / config.K) * np.ones(config.K),
        config.sigma_sq, config.sigma_RIS_sq,
        scsi_bool=0, algo="algo1", state=config.ris_state
    )

    # Assemble pending tasks
    tasks = []
    for param, values in config.varying_params.items():
        for value in values:
            params_combo = {param: value}
            for idx in range(config.NUM_SAMPLES):
                key = make_task_key(idx, params_combo)
                if key not in checkpoint_data:
                    tasks.append((idx, config, channel_samples_range, ris_samples_range,
                                  params_combo, None, None, None, None))

    print(f"Total tasks to run: {len(tasks)}")
    if not tasks:
        print("All tasks already completed in checkpoint.")
        return list(checkpoint_data.values())

    # Execute in parallel, checkpointing after each task
    with Pool(processes=num_processes) as pool:
        for key, out_data, *_ in pool.starmap(simulation, tasks, 
                                               chunksize=max(len(tasks)//num_processes,1)):
            checkpoint_data[key] = out_data
...
if __name__ == '__main__':
    ...
    sim_type, config = select_simulation_type(config)
    config = configure_simulation_params(sim_type, config)
